            ...
            try {
                ...
                const remits = Array.isArray(r.remit_options)
                    ? r.remit_options.map((o) => ({
                        ...
                        client_name: o.client_name || (typeof o.label === "string" ? o.label.split("—")[1]?.trim() : ""),
                    }))
                    : [];
    ...
    return (
        <div>
            ...
            <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 8px" }}>
                ...
                <h1 className="rm-title" style={{ marginTop: 4 }}>DETALLE HOJA DE RUTA</h1>
                ...
                >
                    <HeaderItem label="FECH CREACIÓN" value={loading ? "…" : fmtDate(data?.header?.created_at)} />
                    <HeaderItem label="FECH REPARTO" value={loading ? "…" : fmtDate(data?.header?.delivery_date)} />
                    <HeaderItem label="DESTINO" value={loading ? "…" : (data?.header?.destination || "-")} />
                    <HeaderItem label="CHOFER" value={loading ? "…" : (data?.header?.driver || "-")} />
                    <HeaderItem label="PATENTE" value={loading ? "…" : (data?.header?.plate || "-")} />
                </div>
